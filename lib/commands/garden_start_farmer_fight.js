...
function start_farmer_fight(auto, farmerId, callback)
{
     services.get_garden(function(data){
        ...
        services.get_farmer_oponents(farmerId, function(data){
            var oponents = data.opponents;
    
            if (auto)
            {
                select_enemy_auto(oponents, farmerId, callback);
            }
            else {
                select_enemy_manuel(oponents, farmerId, callback);
            }
        });
    });
}
    ...
    services.login(function(data) {
       var num = argv.number;
        ...
        var callback = function() {
            ...
            if (count < num)
            {
                start_farmer_fight(argv.auto, data.farmer.id, callback);
            }
        }
